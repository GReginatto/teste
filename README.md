...
│       ├── 02.png                 # Histórico de execuções 
│       ├── 03.png                 # Tabela postagens no Supabase 
│       ├── 04.png                 # Tabela leads no Supabase 
...
| `NOTIFICATION_WEBHOOK_URL` | Endpoint de notificação  |
...
**Tabela `leads` no Supabase de registros salvos pelos 5 casos válidos do script:**
...
JSON de entrada
    ...
    → db.insert_lead()   ← backend SQLite / Supabase / PostgreSQL
...
Tradeoff: ligeiramente mais código, mas o comportamento é muito mais usável.
...
Dados digitados em formulários chegam em qualquer capitalização ("dr. JOAO", "dra. ana lima"). `.title()` normaliza para "Dr. Joao" antes de salvar. 
...
Tentativa: o erro `Bad control character in string at position 544` apontava exatamente para o campo com newline. Tentei usar `JSON.stringify()` no Code node para pré-serializar funcionou. 
